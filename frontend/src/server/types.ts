...
export interface Department {
  id: string; // e.g. hành chính, dịch vụ, tố tụng
  name: string;
...
export interface Lawsuit {
  id: string; // e.g. VA-2026-003
  title: string;
  customerId: string;
  lawsuitType: 'Dân sự' | 'Hình sự' | 'Hôn nhân gia đình' | 'Đất đai' | 'Lao động' | 'Kinh doanh thương mại' | 'Khác';
  lawyerId: string; // staffId (luật sư phụ trách)
  supportId?: string; // staffId (nhân viên hỗ trợ)
...
export interface Task {
  id: string; // e.g. CV-001
  title: string;
  assignerId: string; // staffId
  assigneeId: string; // staffId
  departmentId: string;
  customerId?: string;
  profileId?: string; // liên kết hồ sơ dịch vụ
  lawsuitId?: string; // liên kết vụ án
  deadline: string;
...
export interface ChatMessage {
  ...
  channelId: string; // departmentId, staffId (nếu direct chat), profileId, lawsuitId
  senderId: string;
  senderName: string;
  content: string;
  fileUrl?: string;
...
export interface SystemLog {
  ...
  staffName: string;
  action: string;
  target: string; // e.g. "Khách hàng Nguyễn Văn A", "Hồ sơ HS-2026-001"
  timestamp: string;
}
